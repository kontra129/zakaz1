  ...
  <body>
    ...
    if (isset($_POST['change'])) {
      if ($user) {
        ...
        $zan = $_POST['zan'];
        $pay = $_POST['pay'];
        $changeQuery = $conn->query("UPDATE users SET tel = '$tel', zan = '$zan', pay = '$pay' WHERE login = '$login'");

        if ( $changeQuery) {
          echo '<p><div style="color: green;">Данные Успешно изменены, можете вернутся в <a href="index1.php">Личный кабинет</a></div></p>';
        } else {
          echo "Error";
        }
      } else {
