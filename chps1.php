    ...
    if (isset($_POST['cp'])) {
      if ($_POST["new_password"] == $_POST['password_confirmation']) {
        if ($user) {
          ...
          $changeQuery = $conn->query("UPDATE users SET password = '$new_password' WHERE login = '$login'");

          if ( $changeQuery) {
            echo '<p><div style="color: green;">Пароль Успешно изменён, можете вернутся в <a href="index1.php">Личный кабинет</a></div></p>';
          } else {
            echo "Error";
          }
        } else {
          echo '<p><div style="color: red;">Пользователь с таким логином не найден</div></p>';
        }
      } else {
        echo '<p><div style="color: red;">Пароли не совподают</div></p>';
      }
    } else {
